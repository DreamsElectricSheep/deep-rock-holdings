...
*Pulled directly from the private repo's git history. It's a dated snapshot, not a live counter, so treat the date as the freshness bound.*
...
| Apr 2026 | **Multi-agent pipeline hardening.** Capital accounting bug fixed in supervisor (Kelly sizing was cosmetic; a hardcoded value was actually executing). Trade sizes now correctly Kelly-derived end-to-end. |
...
- Recognised that running bots across three exchanges with no unified risk layer was fragile, which led to the multi-agent pipeline with a central risk manager and execution agent
- Identified that overnight Windows/Linux updates could disrupt live trading, so designed the full overnight maintenance system (active hours, auto-login, startup notifications)
- Spotted that ad-hoc Telegram noise from 15+ scripts was making the signal-to-noise ratio unusable, so systematically silenced non-critical alerts and redesigned morning/evening reports
- Noticed that idle capital sitting in SOL between trades was a missed opportunity, so introduced Marinade liquid staking to put it to work
- Recognised that a single bad API response (Jupiter price feed) could corrupt displayed PnL, so rerouted balance reads to the bot's own DB-written state
- Identified that paper bots graduating to live needed a formal gate, so introduced the profitable-sessions canary system before full capital deployment
- Recognised that "the audit found N issues and fixed them" isn't trustworthy on its own, so introduced a forward-return measurement layer so every research signal is scored against what the market actually did next, not against the story told about it afterward
...
The core of the project is a five-agent pipeline where each agent has a distinct role, communicates through shared state (PostgreSQL + JSON), and the whole system runs without human intervention.
...
Sitting alongside this pipeline, not inside it, are three layers that don't touch execution directly but decide what's allowed to reach it: the **Guardian Layer** (kills or halts on hard triggers), the **Board** (a recurring automated audit that proposes and applies code-level changes, described below), and **Signal Measurement** (scores every research call against real forward returns so "this signal source is good" is a tested claim, not an assumption). All three exist because reaching a trade decision through the pipeline above is not the same thing as that decision being trustworthy.
...
|-------|-------|-------------|
| `strategy_analyst.py` | None (no LLM call) | Consumes validated feature vectors from upstream extraction. Composite scoring is fully deterministic; no LLM votes exist in the execution path. |
...
| **The Behavioral Economist** | Market reflexivity, retail feedback loops, mania/panic cascades, operator fatigue | Automation bias; the system trusting its own outputs uncritically |
...
A note on process: several of the board's own premises were **rejected on evidence** during implementation, including proposed deletions that would have broken live components and capital figures that conflated bookkeeping with custody. Every mandate was verified against live system state before execution. That verification step is the actual governing model.
...
Organized by what they trade, not by how well they're doing. The fleet includes strategies that were tested and retired as much as ones still running; that's intentional, since a strategy list with no retirements in it would mean nothing had ever really been checked.
...
| Avail Light Node | v1.13.3, a DA light-client on Avail mainnet, still running and healthy. Its incentive program appears to have been a time-limited campaign rather than an ongoing one, which likely explains the lack of rewards despite uptime. Kept running for the data-availability contribution itself. |
...
This repository contains the README and a curated selection of representative scripts chosen to illustrate architecture and engineering approach. That's deliberate, not an oversight. The tables above name and describe every active module in the private fleet by function, cadence, and role in the pipeline; what they don't include is the code itself.
